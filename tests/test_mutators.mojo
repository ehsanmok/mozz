...
def test_bitflip_output_is_bytes():
    """BitFlip output length must equal input length."""
    var rng = Xoshiro256(seed=1)
    var m = BitFlip()
    var inp = _make_input()
    for _ in range(_RUNS):
        assert_equal(len(m.mutate(Span[UInt8](inp), rng)), len(inp))


def test_bitflip_changes_input():
    ...
    var changed = False
    for _ in range(50):
        var out = m.mutate(Span[UInt8](inp), rng)
...
def test_byte_substitution_range():
...
def test_byte_substitution_uses_boundary():
...
def test_byte_insertion_grows():
    """ByteInsertion must produce output strictly longer than input."""
    var rng = Xoshiro256(seed=5)
    var m = ByteInsertion()
    var inp = _make_input()
    for _ in range(_RUNS):
        assert_true(len(m.mutate(Span[UInt8](inp), rng)) > len(inp))


def test_byte_insertion_max_growth():
    """ByteInsertion must add at most 8 bytes."""
    var rng = Xoshiro256(seed=6)
    var m = ByteInsertion()
    var inp = _make_input()
    for _ in range(_RUNS):
        assert_true(len(m.mutate(Span[UInt8](inp), rng)) <= len(inp) + 8)


def test_byte_deletion_shrinks():
...
def test_byte_deletion_empty_input():
    """ByteDeletion on empty input must return empty output."""
    var rng = Xoshiro256(seed=8)
    var m = ByteDeletion()
    var inp = List[UInt8]()
    assert_equal(len(m.mutate(Span[UInt8](inp), rng)), 0)


def test_block_duplication_grows():
...
def test_splice_output_is_bytes():
...
def test_boundary_int_output_length():
...
def test_boundary_int_uses_boundaries():
...
def test_mutator_chain_output_is_bytes():
    ...
    for _ in range(500):
        var out = chain.mutate(Span[UInt8](inp), rng)
        for i in range(len(out)):
            assert_true(Int(out[i]) >= 0 and Int(out[i]) <= 255)


def test_default_mutator_output_is_bytes():
